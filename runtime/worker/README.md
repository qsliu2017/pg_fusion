...
It attaches to `control_transport`, consumes `protocol` control
...
`protocol::ExecutionOptionsWire` captured at `StartExecution`.
...
```rust,ignore
use worker::{ScanFlowDriver, ScanFlowOpen};

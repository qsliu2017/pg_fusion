...
pub enum DecodedInbound<'a> {
    /// One worker control-plane message decoded from `protocol`.
...
impl WorkerRuntimeCore {
    ...
    ///
    /// `worker` only multiplexes two framed payload families here:
    ...
    pub fn decode_inbound(bytes: &[u8]) -> Result<DecodedInbound<'_>, WorkerRuntimeError> {
        match decode_runtime_message_family(bytes) {
            ...
            Ok(other) => Err(protocol::DecodeError::UnexpectedMessageFamily {
                actual: other as u8,
            }
            .into()),
            Err(runtime_error)
    ...
    fn start_execution(
        ...
        peer: BackendLeaseSlot,
        session_epoch: u64,
        plan: protocol::PlanFlowDescriptor,
        options: protocol::ExecutionOptionsWire,
        scans: protocol::ScanChannelSetRef<'_>,
    ) -> Result<WorkerRuntimeStep, WorkerRuntimeError> {
...
fn materialize_scan_peer_map(
    ...
    for channel in scans.iter() {
        let role = match channel.role {
            protocol::ProducerRole::Leader => scan_flow::ProducerRoleKind::Leader,
            protocol::ProducerRole::Worker => scan_flow::ProducerRoleKind::Worker,
        };
        scan_peers
...
fn backend_lease_slot_from_wire(peer: protocol::BackendLeaseSlotWire) -> BackendLeaseSlot {
...
fn build_worker_planning_session_state() -> SessionState {
    ...
    let mut state = SessionStateBuilder::new()
        ...
        .build();
    let _ = state.register_udf(df_functions::pg_boolout_udf());
...
impl TransportWorkerRuntime {
    ...
    /// Encode and send one `protocol` worker-to-backend message.
...
mod tests {
    ...
    fn accept_from_peer(
        ...
        let mut encoded = vec![0_u8; protocol::encoded_len_backend_execution_to_worker(message)];
        let written = protocol::encode_backend_execution_to_worker_into(message, &mut encoded)
            .expect("encode backend control");
        let decoded = protocol::decode_backend_execution_to_worker(&encoded[..written]).unwrap();
    ...
    fn scan_channel(scan_id: u64, peer: BackendLeaseSlot) -> ScanChannelDescriptorWire {
        ScanChannelDescriptorWire {
            scan_id,
            producer_id: 0,
            role: protocol::ProducerRole::Leader,
    ...
    fn decode_inbound_prefers_runtime_protocol_control() {
        ...
        let mut encoded = vec![0_u8; protocol::encoded_len_backend_execution_to_worker(message)];
        let written =
            protocol::encode_backend_execution_to_worker_into(message, &mut encoded).unwrap();
    ...
    fn slot_reuse_by_different_backend_peer_accepts_epoch_one_again() {
        ...
        let mut encoded_a = vec![0_u8; protocol::encoded_len_backend_execution_to_worker(start_a)];
        let written_a =
            protocol::encode_backend_execution_to_worker_into(start_a, &mut encoded_a).unwrap();
        backend_a
            .to_worker_tx()
        ...
        let mut encoded_b = vec![0_u8; protocol::encoded_len_backend_execution_to_worker(start_b)];
        let written_b =
            protocol::encode_backend_execution_to_worker_into(start_b, &mut encoded_b).unwrap();
        backend_b
            .to_worker_tx()

...
impl PlanDecodeSession {
    ...
    pub fn new() -> Self {
        ...
        let _ = FunctionRegistry::register_udf(&mut ctx, df_functions::pg_bpchar_length_udf());
...
fn decode_pg_scalar_udf(name: &str) -> Option<Arc<ScalarUDF>> {
    ...
    if name.eq_ignore_ascii_case("pg_fusion_numeric_round_scale") {
        return Some(df_functions::pg_numeric_round_scale_udf());
    }
    ...
    if name.eq_ignore_ascii_case("pg_fusion_bpchar_cmp_key") {
        return Some(df_functions::pg_bpchar_cmp_key_udf());
    }
    if name.eq_ignore_ascii_case("pg_fusion_bpchar_length") {
        return Some(df_functions::pg_bpchar_length_udf());
    }
